...
const ISF = () => {
  ...
  useEffect(() => {
    runSafeAsync(() => fetchDataForDays(Math.max(analysisPeriod, 7)), { label: 'ISF initial fetch' });
  ...
  const convertedAnalysisResults = React.useMemo(() => {
    ...
    const convertIsfArray = (isfArray: unknown): IsfLike[] => {
    ...
    return {
      ...
      currentProfile: {
        ...
        sens: convertIsfArray(analysisResults.currentProfile?.sens)
      },
  ...
  const handleTimeWindowChange = (value: string) => {
    if (value === 'custom') {
      ...
      if (daysNeeded > analysisPeriod) {
        runSafeAsync(() => fetchDataForDays(Math.min(daysNeeded, 90)), { label: 'ISF fetch more data for time window' });
      }
    }
  };
  ...
  const handleCustomDateSubmit = () => {
    ...
    runSafeAsync(() => fetchDataForDays(Math.min(daysToFetch, 90)), { label: 'ISF fetch data for custom range' });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center pb-4 border-b border-gray-200 dark:border-gray-700">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">AI-Enhanced ISF Analysis</h2>
          ...
          <button 
            onClick={() => {
              if (isCustomRange) {
                ...
                runSafeAsync(() => fetchDataForDays(Math.max(daysNeeded, analysisPeriod)), { label: 'ISF refresh fetch data' });
              }
            }}
      ...
      {!analysisResults && !analyzing && (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md text-center">
          ...
          <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">AI-Enhanced ISF Analysis</h3>
          <p className="text-gray-600 dark:text-gray-400 mb-4">
            {(!filteredReadings.length || !filteredTreatments.length) ? (
              ...
              `Loading ISF analysis for the last ${analysisPeriod} day${analysisPeriod > 1 ? 's' : ''}...`
            )}
          </p>
      ...
      <div className="space-y-6">
        <SuggestionTable
          title="Ultra-Safe ISF Recommendations"
          currentValues={convertedAnalysisResults.currentProfile.sens}
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md transition-colors duration-200">
          ...
          <div className="space-y-4">
            <p className="text-gray-700 dark:text-gray-300">
              These ultra-conservative insulin sensitivity factor (ISF) suggestions prioritize preventing hypoglycemia above all else. 
              The analysis uses a safety-first approach with:
            </p>
            <ul className="list-disc list-inside text-gray-700 dark:text-gray-300 space-y-2">
              <li>Maximum 5% adjustments (reduced from previous 15%)</li>
              <li>Preference for less aggressive ISF values when hypoglycemia risk exists</li>
            ...
            <div className="bg-yellow-50 dark:bg-yellow-900/20 p-4 rounded-md mt-4">
              <div className="flex items-start">
                ...
                <p className="text-yellow-800 dark:text-yellow-200 text-sm">
                  <strong>Important:</strong> Always consult with your healthcare provider before making changes to your ISF settings.
